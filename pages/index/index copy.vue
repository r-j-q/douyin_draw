<template>
	<view class="ji">
		...
		activity_id===	{{activity_id}}
			...
			<button type="primary" v-if="ifShareVideo" class="button-video" @click="getVideo">发布视频</button>
		</view>
		...
		<view class="" v-if="gameType==2">
			...
			<button type="primary" class="button-video-style" @click="getVideo">发布视频</button>
...
<script module="renderScript" lang="renderjs">
	...
	// import html2canvas from "@/components/x-Longscree/x-Longscree.vue"
	export default {
		components: {
			gameg
		},
		...
		onLoad(options) {
			...
			this.getChoujiangYouXi(options.activity_id);
		...
		methods: {
			...
			getChoujiangYouXi(activity_id) {
				console.log("是否有activity_id", activity_id)
				this.$Z.post(getApp().globalData.reqUrl + "/activity/game", {
			...
			async getVideo() {
				...
				this.$Z.post(getApp().globalData.reqUrl + "/activity/index", {
					activity_id: this.activity_id
				}, {
					native: false
				}).then(res => {
					if (systemPhone.platform == 'ios') {
						window.open(res.data.schema)
					} else {

						window.location.href = `${res.data.schema}`
					}
			...
			async getTrimVideo() {
				...
				this.$Z.post(getApp().globalData.reqUrl + "/activity/trimVideo?id=1", {}, {
					native: false
				}).then(res => {
					//异步操作成功
					// console.log('==========...', res)
